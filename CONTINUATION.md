...
**Estado al cerrar esta sesión (2026-08-08):** CI en GitHub Actions (corre la suite en cada push/PR a `main`), reporte de errores de producción a Firestore (`_logClientError` + visor en Ajustes), y catálogo de ejercicios reutilizables por equipo (`S.drills`) adjuntables a la sesión del pase de lista. Testeado (334/334 en 31 archivos). **Pendiente de decisión de Mario antes de publicar:** estos cambios están hechos y verificados en local/sandbox pero NO se han pusheado a `origin/main` todavía — el deploy a GitHub Pages es automático en cuanto se hace push, así que se dejó sin pushear para que Mario revise antes de que llegue a producción real. Si Mario confirma, hace falta además `bash scripts/build-test-deploy.sh` para sincronizar `/test/`.
