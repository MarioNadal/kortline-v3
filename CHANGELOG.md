...
- **B-SYNC2** (el bug del cumpleaños, reportado explícitamente). Si cambiabas un dato de un jugador (p. ej. su cumpleaños) sin entrar al equipo, no se actualizaba en otros dispositivos. Causa: desde B-SYNCATT1 (v3.0.0-dev.54), la asistencia y las notas de entrenamiento se escuchan en tiempo real para TODOS los equipos del club a la vez, precisamente para evitar este problema -- pero "jugadoras" y "partidos" se dejaron fuera de ese mecanismo a propósito, bajo la idea de que casi siempre se editan estando dentro de ese equipo. Esa idea dejó de ser cierta en cuanto la pantalla "Hoy" empezó a leer jugadoras (aviso de cumpleaños, B-BDAY1) y partidos (sección "🏀 Partidos") de TODOS los equipos, se estén viendo o no: si un dispositivo no había entrado a un equipo concreto en esa sesión, su copia local de las jugadoras/partidos de ese equipo se quedaba anticuada. Ahora "jugadoras" y "partidos" se escuchan en tiempo real para todos los equipos a la vez, exactamente igual que la asistencia -- "eventos" y "ejercicios" siguen por equipo bajo demanda, porque nada fuera de la propia pantalla del equipo los necesita.
- **B-SYNC4** (muy probablemente la causa principal de "a veces no llega la asistencia a otro móvil", encontrado durante esta revisión, no reportado explícitamente por nadie). `_diffSessions()` -- la función que decide qué asistencia/notas de entrenamiento hay que subir a Firestore en cada sincronización -- solo se defendía de recibir `undefined` al construir la lista de fechas a comparar, pero luego las leía sin esa protección. `_lastSyncedSnapshot` (con qué se compara) arranca en `null` en CADA apertura de la app -- vive solo en memoria, nunca en localStorage -- y sigue en `null` hasta que algo remoto cambie de verdad o hasta que una subida tenga éxito. Si lo primero que hacía un entrenador al abrir la app era pasar lista de un equipo con historial de asistencia ya existente (el caso normal: casi siempre hay historial) y nada había cambiado remotamente desde la última vez, esa primerísima sincronización de la sesión reventaba con un error silencioso (capturado, solo un aviso por consola) -- la asistencia se quedaba a salvo en ESE dispositivo pero nunca llegaba a Firestore, así que ningún otro móvil la veía. Corregido dando a `curSess`/`prevSess` el mismo `||{}` que ya tenía la construcción de claves.
- **B-SYNC3** (red de seguridad para cualquier fallo de sincronización que quede, incluido el propio B-SYNC4 mientras alguien no haya actualizado): antes, si UN intento de sincronización fallaba por el motivo que fuera, el error solo se registraba por consola y no se reintentaba hasta que el entrenador tocase otra cosa en la app. Si cerraba la app justo después de ese fallo (el caso típico: pasar lista y salir del pabellón con mala cobertura), ese cambio podía quedarse sin sincronizar indefinidamente. Ahora un fallo programa un reintento automático (reutilizando el propio mecanismo de espera mínima entre envíos, ~3s), y además se comprueba activamente si hay algo pendiente de subir en cuanto el dispositivo recupera conexión (evento `online`) y en cuanto la app vuelve a primer plano (`visibilitychange`) -- sin depender de que el usuario haga ningún cambio nuevo.

### Probado (jsdom)

- `tests/session_sync_all_teams.test.js` ampliado (10 comprobaciones nuevas, B-SYNC2): listener de jugadoras y de partidos para un equipo NO abierto (antes 0), eventos/ejercicios siguen sin escucharse hasta entrar al equipo, un cumpleaños de hoy de una jugadora de un equipo no abierto aparece en "Hoy" sin entrar a ese equipo, lo mismo para un partido de hoy, y que al borrarse un equipo se cortan también sus listeners nuevos.
- `tests/cloud_sync_first_run.test.js` nuevo (6 comprobaciones, B-SYNC4): reproduce exactamente el escenario real (asistencia ya existente + `_lastSyncedSnapshot` a null) y confirma que la primera sincronización de la sesión ya no revienta y sí llega a Firestore, tanto la asistencia de hoy como la de un día anterior y las notas de entrenamiento; una segunda sincronización sin cambios no reenvía nada de más.
- `tests/cloud_sync_retry.test.js` nuevo (7 comprobaciones, B-SYNC3): un fallo de sincronización se reintenta solo sin que el usuario haga nada más, el evento `online` y `visibilitychange` disparan una comprobación de lo pendiente, y el camino feliz (sin fallos) sigue funcionando igual que antes.
- Los tres fixes se verificaron además revirtiéndolos uno a uno: las comprobaciones nuevas relevantes de cada uno fallan sin su fix correspondiente.
...
- **B-GUEST7**. Reportado por Mario (Cadete Masculino, 21/09): al seleccionar jugadores puntuales ya existentes en "🔄 + Invitado puntual" → "Más frecuentes" y pulsar "Añadir seleccionados", su asistencia se guardaba correctamente por dentro, pero el jugador desaparecía sin más del pase de lista de ese día — "se pierden en el vacío". Causa: `_isPlayerActiveOn()` ya decidía correctamente si un puntual de entreno "cuenta" en una fecha dada mirando `p.attDates` (todos los días en los que se ha reutilizado, desde B-GUEST4), pero **tres sitios** que necesitan la misma decisión (el propio pase de lista `att()`, la tarjeta de cada equipo en la pantalla "Hoy", y el resumen de WhatsApp `buildDailyText`) nunca se migraron a esa función centralizada y seguían comparando solo `p.addedAt===fecha` — la fecha de ALTA, que nunca cambia al reutilizar al jugador otro día distinto. En cuanto se reutilizaba un puntual desde "Más frecuentes" (o al volver a escribir su nombre a mano) en un día posterior a su alta, esos tres sitios dejaban de reconocerlo aunque su sesión sí tuviera el dato guardado. Se extrajo un helper único (`_attOnlyActiveOn`, usado también ahora dentro de `_isPlayerActiveOn`) y se corrigieron los tres sitios para usarlo. Retrocompatible: un puntual muy antiguo sin el campo `attDates` sigue funcionando exactamente igual que antes (fallback a `addedAt`).

### Probado (jsdom)

- `tests/guest_att_only.test.js` ampliado (14 comprobaciones nuevas): reproduce el escenario exacto (alta un día, reutilización desde "Más frecuentes" una semana después) y comprueba que el puntual reutilizado aparece en `att()` y en `buildDailyText` del nuevo día, que sigue apareciendo también en su día de alta original, que un día en el que nunca se le añadió sigue sin mostrarlo, y que un puntual legado sin `attDates` mantiene el comportamiento previo (solo su día de alta). Verificado además que, revirtiendo el fix, estas dos comprobaciones fallan — confirma que el test detecta el bug real.
- Suite completa: 1081/1081.
- CACHE_VERSION → `kortline-v3.0.0-dev.74`. APP_VERSION sincronizada.
...
- `tests/session_sync_all_teams.test.js` (nuevo, 18 comprobaciones): usa un mock mínimo de Firestore inyectado en el harness (`loadApp({firebase})`, nuevo parámetro opcional — quita las etiquetas `<script>` del SDK real de Firebase del HTML antes de cargarlo en jsdom, para no depender de red ni arriesgarse a tocar el proyecto real) para comprobar que, con dos equipos en el club y viendo solo uno de ellos, hay un listener de asistencia conectado para AMBOS equipos; que una asistencia de un día cualquiera escrita en el equipo que no se está viendo llega igualmente a `S.sessions`; que lo mismo ocurre con las notas de entrenamiento; que borrar un equipo del club desconecta su listener de asistencia sin afectar al del otro; y que renderizar/navegar no duplica listeners ya conectados. **Comprobación añadida a petición del usuario** ("puedes probarlo es que antes no lo hacia"): con la pantalla "Hoy" ya renderizada y sin tocar nada, la tarjeta de un equipo que no es el que se está viendo (con entrenamiento programado hoy) pasa sola de "Pendiente" a "✓ Pasada" en cuanto llega la asistencia de ese equipo por el listener — comprobando el HTML real de la pantalla, sin llamar a `render()` a mano ni navegar a "pasar lista". Esta zona no tenía ninguna cobertura de test antes (los 613 tests existentes cargan `index.html` sin `firebase`, en modo local).
- Suite completa: 632/632 en 48 archivos.
- CACHE_VERSION → `kortline-v3.0.0-dev.54`. APP_VERSION sincronizada. Sin cambios en `index.html`/`sw.js` respecto al commit anterior — solo se amplía el test, no hace falta volver a subir el número de build ni regenerar `/test/`.
...
- **B-DRILLSAVE1** (pendiente de la sesión anterior, sin llegar a documentarse en CONTINUATION.md antes de esta). `save()` — la función que vuelca `S` a `localStorage` en cada acción — no incluía la clave `"cbj:drills"`, a diferencia de `load()` y de `_persistLocalCacheOnly()` (el volcado que sí se dispara al recibir cambios de Firestore), que sí la tenían. En la práctica esto era invisible casi siempre porque Firestore reescribe `S.drills` en cuanto sincroniza — pero si un entrenador editaba el catálogo de ejercicios del equipo (`saveDrill`/`deleteDrill`, ambas llaman a `save()`) estando sin cobertura, y refrescaba la página o cerraba la pestaña antes de que la sincronización con la nube llegara a completarse, esa edición se perdía por completo al recargar: `load()` reconstruía `S.drills` desde `localStorage`, que nunca había llegado a recibir el cambio. El resto del estado (equipos, jugadores, sesiones, eventos, notas de entrenamiento...) sí sobrevivía a ese mismo escenario porque sus claves sí estaban en `save()`. Se añade `lsSet("cbj:drills",S.drills)` a `save()`, siguiendo el mismo patrón que ya usan `sessions`/`events`/`trainingNotes`.
- **B-RIVALPICK1** (pendiente de la sesión anterior, sin confirmar; investigado a fondo antes de tocar nada, como se pidió — y resultó ser un bug real, más amplio de lo sospechado). La sospecha era sobre faltas individuales de jugadoras rivales; el bug real afecta a **cualquier** acción individual (puntos, rebotes, asistencias, robos, tapones, pérdidas y faltas), no solo faltas. `openActionPicker()` — el modal "¿Quién?" que se abre al tocar un botón de acción en el partido en vivo — construía su lista de jugadoras siempre a partir de nuestra propia plantilla (`pl(S.teamId)`) y de `m.live.onCourt`, sin mirar nunca `m.live.activeTeam`. Con la plantilla del rival completa (5+, modo de seguimiento individual — el caso con menos de 5 ya estaba cubierto desde dev.38 por el "modo equipo" de B-RIVAL1), estar en la pestaña Rival y tocar cualquier botón de acción abría el picker con **nuestras** jugadoras en pista, nunca con las del rival — cualquier estadística del rival podía acabar apuntada, por error, a una jugadora nuestra. `liveAction()` en sí ya distinguía bien el pid rival una vez elegido (vía `isRivalPid`); el hueco estaba enteramente en qué lista ofrecía el picker. El test de regresión de B-RIVAL1 (`rival_team_mode.test.js`, caso C, "sin regresión") solo comprobaba que el HTML seguía conteniendo la cadena `"openActionPicker("` con plantilla rival completa — nunca comprobó qué jugadoras aparecían dentro. Se corrige `openActionPicker()` para que, cuando `m.live.activeTeam==="rival"`, use `m.rivalPlayers` y `m.live.rivalOnCourt` (con sus stats de `m.live.rivalStats`) en vez de nuestra plantilla — igual criterio que ya usan `rivalCourtCards`/`rivalBenchCards`/`openCourtRivalSubModal` en el resto de esa misma pantalla.

### Probado (jsdom)

- `tests/drills.test.js` ampliado (3 comprobaciones nuevas): `save()` escribe `"cbj:drills"` en `localStorage` con el contenido actual del catálogo, y un `load()` posterior (simulando un refresco de página) recupera un ejercicio guardado que antes del fix se habría perdido.
- `tests/rival_individual_picker.test.js` (16 comprobaciones nuevas): con plantilla rival completa, el picker de una acción de puntos y el de una falta ofrecen jugadoras del rival (y ya no ofrecen ninguna de las nuestras); elegir una jugadora rival desde el picker registra correctamente en `live.rivalStats`/`live.rivalFouls` sin tocar `live.stats`/`live.teamFouls`; y de vuelta en nuestra propia pestaña el picker sigue ofreciendo nuestra plantilla con normalidad (sin regresión).
- Suite completa: 613/613 en 47 archivos.
- CACHE_VERSION → `kortline-v3.0.0-dev.53`. APP_VERSION sincronizada.
